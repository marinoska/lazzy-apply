...
export class Client {
	...
	public async setAccessToken() {
		// Get session from Chrome storage since persistSession is false
		const result = await chrome.storage.local.get("supabaseSession");
		const session = result?.supabaseSession as { access_token?: string } | null;
	...
	public deleteAccessToken() {
		this.authHeader.Authorization = undefined;
		this._authToken = "";
	}
