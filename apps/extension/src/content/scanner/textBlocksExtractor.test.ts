...
import { extractTextBlocks, type TextBlock } from './textBlocksExtractor';
...
describe('extractTextBlocks', () => {
  ...
  it('should extract all header levels (h1-h6)', () => {
    ...
    expect(headers.map(h => h.element)).toEqual(['h1', 'h2', 'h3', 'h4', 'h5', 'h6']);
  });
  ...
  it('should filter out short paragraphs but not headers', () => {
    ...
    expect(blocks.some(b => b.text === 'Short' && b.type === 'header')).toBe(true);
  ...
  it('should maintain order: headers first, then paragraphs', () => {
    ...
    expect(headerIndices[0]).toBeLessThan(blocks.findIndex(b => b.type === 'paragraph'));
  });
  ...
  it('should skip content inside script tags', () => {
    ...
    expect(blocks.some(b => b.text.includes('main job description'))).toBe(true);

    // Should NOT extract script content
    expect(blocks.some(b => b.text.includes('This should not be extracted'))).toBe(false);
  ...
  it('should skip content in footer elements', () => {
    ...
    expect(blocks.some(b => b.text.includes('Footer div content'))).toBe(false);
  });
  ...
  it('should extract company name and position from builtin.com job card', () => {
    ...
    const positionHeader = headers.find(h => h.text === 'Data Engineering Functional Lead');
    ...
    expect(headers.some(h => h.text === 'Data Engineering Functional Lead')).toBe(true);
  });
});
