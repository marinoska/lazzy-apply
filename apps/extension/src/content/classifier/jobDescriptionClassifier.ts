...
import {
	detectApplyButtons,
	type ApplyButtonDetectionResult,
...
export function classifyJobDescription(text: string): ClassificationResult {
	...
	const signals: ClassificationSignals = {
		responsibility_language: matchesAnyPattern(text, [
			...responsibilityExpressions,
			...obligationWords,
		]),
		requirement_language: matchesAnyPattern(text, [
			...requirementTerminology,
			...skillTerminology,
		]),
		candidate_framing: matchesAnyPattern(text, candidatePhrases),
		action_verbs: matchesAnyPattern(text, [
			...universalActionVerbs,
			...operationalVerbs,
		]),
		soft_skills: matchesAnyPattern(text, [
			...universalSoftSkills,
			...environmentTerms,
		]),
	...
	const isJobDescription =
		...
		positiveSignals >= MIN_SIGNALS_REQUIRED;
...
export function classifyParagraphs(
	...
	const examinedCount = paragraphs.filter(
		(p) => p.trim().length > MIN_PARAGRAPH_LENGTH,
	).length;
	console.log(
		`Examining ${examinedCount} paragraphs out of ${paragraphs.length} total`,
	);
...
function aggregateSignals(results: ClassificationResult[]): {
	...
	const avgSignalsPerParagraph =
		signalCounts.reduce((a, b) => a + b, 0) / totalParagraphs;

	// Normalize to 0-1 range (assuming EXPECTED_SIGNALS_PER_PARAGRAPH+ signals per paragraph is very strong)
	const signalDensity = Math.min(
		avgSignalsPerParagraph / EXPECTED_SIGNALS_PER_PARAGRAPH,
		1,
	);
...
export function classifyDocument(rawParagraphs: string[]): {
	...
	const sectionResults = classifySections(
		paragraphs.filter((p) => p.length > MIN_PARAGRAPH_LENGTH),
		SECTION_SIZE,
	);
	const jobDescriptionSections = sectionResults.filter(
		(r) => r.isJobDescription,
	).length;
	...
	const dominantSignals = Object.entries(totalSignals)
		...
		.sort(
			(a, b) =>
				totalSignals[b.replace(/ /g, "_") as keyof ClassificationSignals] -
	...
	const sectionRatio =
		jobDescriptionSections / Math.max(sectionResults.length, 1);
